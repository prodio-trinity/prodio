...
export function OrderDetail({ id }: { id: string }) {
  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.detailGrid}>
      ...
      <article className={styles.card}><h2>진행 정보</h2><dl className={styles.details}><div><dt>상태</dt><dd><span className={`${styles.badge} ${styles[`status${order.status}`]}`}>{ORDER_STATUS_LABELS[order.status]}</span></dd></div><div><dt>등록자 ID</dt><dd>{order.createdBy}</dd></div><div><dt>등록일</dt><dd>{new Date(order.createdAt).toLocaleString("ko-KR")}</dd></div>{order.cancellationReason && <div><dt>취소 사유</dt><dd>{order.cancellationReason}</dd></div>}</dl></article>
