...
export function OrderList() {
  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.card}>
      ...
      <div className={styles.tableWrap}><table><thead><tr><th>주문번호</th><th>수주처 / 품목</th><th>수량</th><th>금액</th><th>납기</th><th>상태</th></tr></thead><tbody>{data.orders.map(order => <tr key={order.id}><td><Link href={`/orders/${order.id}`}>#{order.id}</Link></td><td><strong>{order.clientName}</strong><span>{order.productName}</span></td><td>{order.quantity.toLocaleString()}</td><td>{order.totalAmount.toLocaleString()}원</td><td>{order.dueDate}</td><td><span className={styles.badge}>{ORDER_STATUS_LABELS[order.status]}</span></td></tr>)}</tbody></table></div>}
