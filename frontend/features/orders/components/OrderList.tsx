...
export function OrderList() {
  ...
  return <main className={styles.shell}>
    <header className={styles.header}><div><span className={styles.eyebrow}>ORDER</span><h1>주문 관리</h1><p>접수된 주문의 내용, 입금 확정과 취소를 관리합니다.</p></div></header>
    ...
    <section className={styles.card}>
      ...
      <div className={styles.tableWrap}><table><thead><tr><th>주문번호</th><th>수주처 / 주문서명</th><th>수량</th><th>금액</th><th>상태</th><th>배송 현황</th><th>작업</th></tr></thead><tbody>{data.orders.map(order => <tr key={order.id}><td><Link href={`/orders/${order.id}`}>#{order.id}</Link></td><td><strong>{order.orderName || order.clientName}</strong><span>{order.clientName} · {order.items.map((item) => item.productName).join(", ")}</span></td><td>{order.items.reduce((sum, item) => sum + item.quantity, 0).toLocaleString()}</td><td>{order.totalAmount.toLocaleString()}원</td><td><span className={`${styles.badge} ${styles[`status${order.status}`]}`}>{ORDER_STATUS_LABELS[order.status]}</span></td><td>{order.status === "CONFIRMED" ? <ShippingStatusButton orderId={order.id} /> : "-"}</td><td>{order.status === "PENDING_PAYMENT" ? <button type="button" className={styles.smallButton} onClick={() => openStatusModal(order)}>상태 변경</button> : "-"}</td></tr>)}</tbody></table></div>}
