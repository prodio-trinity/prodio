...
export function MyOrderDetail({ id }: { id: string }) {
  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.progressGrid}>
      <article className={styles.progressItem}><span>주문</span><strong className={`${styles.badge} ${styles[`status${order.status}`]}`}>{ORDER_STATUS_LABELS[order.status]}</strong></article>
    ...
    <section className={styles.detailGrid}>
      ...
      <article className={styles.card}><h2>배송 및 결제</h2><dl className={styles.details}><div><dt>배송지</dt><dd>{order.delivery.name}</dd></div><div><dt>배송 주소</dt><dd>{order.delivery.postalCode} {order.delivery.addressLine1} {order.delivery.addressLine2}</dd></div><div><dt>결제 상태</dt><dd>{order.status === "CONFIRMED" ? "확인" : order.status === "CANCELLED" ? "취소" : "미확인"}</dd></div><div><dt>등록일</dt><dd>{new Date(order.createdAt).toLocaleString("ko-KR")}</dd></div>{order.cancellationReason && <div><dt>취소 사유</dt><dd>{order.cancellationReason}</dd></div>}</dl></article>
